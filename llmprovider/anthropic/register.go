...
func init() {
	registry.Register(provider.ProtocolAnthropicMessages, factory)
}
...
func factory(def provider.Definition, config types.ModelProviderConfig, store *auth.Store, _ float32) (cogito.LLM, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("anthropic: resolve credentials: %w", err)
	}
	if resolved.APIKey == "" {
		return nil, fmt.Errorf("anthropic: no credentials — run 'nib login anthropic' or set %s", def.EnvVar)
	}
	return New(Config{
		Model:   config.Model,
